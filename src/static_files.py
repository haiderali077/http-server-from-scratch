...
def resolve_filename(path):
    """Keep the existing cwd-relative path rules during this refactor.

    A document root and safe path containment are upcoming roadmap items.
    Replacing '..' is retained for compatibility, not considered secure.
    """
...
def serve_file(request):
    ...
    try:
        filename = resolve_filename(request.path)
