...
def measure(port, concurrency, requests, keep_alive, expected):
    def worker(count):
        client = None
        samples = []
        try:
...
def main():
    ...
    with context as server:
        ...
        result["warmup"] = measure(port, args.connections, args.warmup, args.keep_alive, payload)
        result["runs"] = [measure(port, args.connections, args.requests, args.keep_alive, payload)
